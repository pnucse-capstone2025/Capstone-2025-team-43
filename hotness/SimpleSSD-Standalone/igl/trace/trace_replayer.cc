...
/* if (groupID[ID_TEMPERATURE] > 0 && match.size() > groupID[ID_TEMPERATURE]) {
    linedata.temperature = std::stoi(match[groupID[ID_TEMPERATURE]]);
  } else {
    linedata.temperature = 1; // 기본값: medium
  } */

int temp_raw = 1;  // 기본값
if (groupID[ID_TEMPERATURE] > 0 && match.size() > groupID[ID_TEMPERATURE]) {
  const std::string s = match[groupID[ID_TEMPERATURE]].str();
  char* end = nullptr;
  long v = std::strtol(s.c_str(), &end, 10);
  if (end != s.c_str() && *end == '\0') {
    temp_raw = static_cast<int>(v);
  } else {
    // 헤더("Hotness_Label") 등 비숫자면 기본값 유지(원하면 경고 로그)
  }
}
// 온도 대입 직후
  linedata.temperature = temp_raw;
// after: linedata.temperature = temp_raw;
...
void TraceReplayer::submitIO() {
  ...
  bio.temperature = linedata.temperature;
