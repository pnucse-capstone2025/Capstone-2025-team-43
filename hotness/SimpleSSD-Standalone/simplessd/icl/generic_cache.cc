...
namespace ICL {
  static GenericCache* g_last_generic_cache = nullptr;   // 내부 저장소
  GenericCache* getLastGenericCache() {                  // ★ 자유 함수 정의
    return g_last_generic_cache;
  }
...
GenericCache::~GenericCache() {
  ...
  if (g_last_generic_cache == this)     // ★ 안전하게 해제
    g_last_generic_cache = nullptr;
}
  ...
  for (uint32_t row = 0; row < lineCountInSuperPage; row++) {
    for (uint32_t col = 0; col < parallelIO; col++) {
      ...
      if (evictData[row][col]->valid && evictData[row][col]->dirty) {
        ...
        // 1) 캐시 메타에 저장돼 있으면 그것을 복원
        auto it = lineTemp.find(evictData[row][col]);
        if (it != lineTemp.end()) {
          reqInternal.temperature = it->second;
        } 
        
        pFTL->write(reqInternal, beginAt);
        //추가
        lineTemp.erase(evictData[row][col]);
      }
...
bool GenericCache::write(Request &req, uint64_t &tick) {
  ...
  if (useWriteCaching) {
    ...
    if (wayIdx != waySize) {
      ...
      // Update last accessed time, 추가함
      if (dirty) {
        lineTemp[&cacheData[setIdx][wayIdx]] = (uint8_t)req.temperature;
      } else {
        lineTemp.erase(&cacheData[setIdx][wayIdx]);
      }
...
void GenericCache::flushAll(uint64_t &tick) {
  if (!(useReadCaching || useWriteCaching)) return;
  LPNRange all{0, std::numeric_limits<uint64_t>::max()};
  flush(all, tick);
}
