...
int main(int argc, char *argv[]) {
  ...
  // sim/main.cc, main() 함수 안에서 engine/simConfig/statEvent 정의된 '아래' 위치에 둬야 합니다.
  std::function<void()> endCallback = []() {
  // 주기 통계 이벤트가 켜져 있으면 해제
    if (simConfig.readUint(CONFIG_GLOBAL, GLOBAL_LOG_PERIOD) > 0) {
      engine.descheduleEvent(statEvent);
    }

  // 1) 캐시 엔드-플러시
    uint64_t t = engine.getCurrentTick();
    if (auto* gcache = SimpleSSD::ICL::getLastGenericCache()) {
      gcache->flushAll(t);
    }

  // 2) stop을 미래 tick에 '이벤트'로 예약 (콜백 내에서 drain/re-enter 금지)
    SimpleSSD::Event stopEv = engine.allocateEvent([](uint64_t) {
      engine.stopEngine();
    });
    engine.scheduleEvent(stopEv, t + 1000000000ULL); // +1s (ps 단위면 1e12로)
  };
