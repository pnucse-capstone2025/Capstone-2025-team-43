...
bool PageMapping::initialize() {
  ...
  // WAF monitoring initialization message
  printf("\n=== WAF Real-time Monitoring Started ===\n");
  printf("Monitor will update every 1000 writes and after each GC event\n");
  printf("WAF = Write Amplification Factor (Device Writes / Host Writes)\n\n");
  fflush(stdout);
...
void PageMapping::write(Request &req, uint64_t &tick) {
  ...
  if (req.ioFlag.count() > 0) {
    // Update host write bytes counter
    stat.host_write_bytes += req.ioFlag.count() * (param.pageSize / param.ioUnitInPage);
    
    writeInternal(req, tick);

    // Real-time WAF monitoring - print every 1000 writes
    static uint64_t write_count = 0;
    write_count++;
    
    if (write_count % 1000 == 0) {
      double hostB = std::max<uint64_t>(1, stat.host_write_bytes);
      double waf_total = double(stat.device_write_bytes) / hostB;
      double waf_gc = double(stat.device_gc_write_bytes) / hostB;
      
      printf("\r\33[2K[WAF Monitor] Total: %.3f | GC: %.3f | GC Count: %" PRIu64 " | Host: %.1fMB | Device: %.1fMB",
             waf_total, waf_gc, stat.gcCount, 
             hostB / (1024.0 * 1024.0), 
             double(stat.device_write_bytes) / (1024.0 * 1024.0));
      fflush(stdout);
    }
...
void PageMapping::doGarbageCollection(std::vector<uint32_t> &blocksToReclaim,
  ...
  for (auto &iter : writeRequests) {
    ...
    // Update GC write bytes counters
    uint32_t writeBytes = iter.ioFlag.count() * (param.pageSize / param.ioUnitInPage);
    stat.device_write_bytes += writeBytes;
    stat.device_gc_write_bytes += writeBytes;
...
void PageMapping::writeInternal(Request &req, uint64_t &tick, bool sendToPAL) {
  ...
  for (uint32_t idx = 0; idx < bitsetSize; idx++) {
    if (req.ioFlag.test(idx) || !bRandomTweak) {
      ...
      if (sendToPAL) {
        ...
        // Update device write bytes counters
        uint32_t writeBytes = palRequest.ioFlag.count() * (param.pageSize / param.ioUnitInPage);
        stat.device_write_bytes += writeBytes;
        stat.device_user_write_bytes += writeBytes;
      }
  ...
  if (freeBlockRatio() < gcThreshold) {
    ...
    // Real-time WAF monitoring after GC
    double hostB = std::max<uint64_t>(1, stat.host_write_bytes);
    double waf_total = double(stat.device_write_bytes) / hostB;
    double waf_gc = double(stat.device_gc_write_bytes) / hostB;
    
    printf("\r\33[2K[GC Event] WAF Total: %.3f | GC: %.3f | GC Count: %" PRIu64 " | Blocks Reclaimed: %zu\n",
           waf_total, waf_gc, stat.gcCount, list.size());
    fflush(stdout);
  }
}
...
void PageMapping::getStatList(std::vector<Stats> &list, std::string prefix) {
  ...
  // Byte counters for tracking Write Amplification
  temp.name = prefix + "page_mapping.bytes.host_write";
  temp.desc = "Total bytes written by host";
  list.push_back(temp);

  temp.name = prefix + "page_mapping.bytes.device_write";
  temp.desc = "Total bytes written to device";
  list.push_back(temp);

  temp.name = prefix + "page_mapping.bytes.device_user_write";
  temp.desc = "Total user bytes written to device";
  list.push_back(temp);

  temp.name = prefix + "page_mapping.bytes.device_gc_write";
  temp.desc = "Total GC bytes written to device";
  list.push_back(temp);
...
void PageMapping::getStatValues(std::vector<double> &values) {
  ...
  // Push byte counters
  values.push_back(stat.host_write_bytes);
  values.push_back(stat.device_write_bytes);
  values.push_back(stat.device_user_write_bytes);
  values.push_back(stat.device_gc_write_bytes);

  // Calculate and push Write Amplification Factors
  double hostB = std::max<uint64_t>(1, stat.host_write_bytes);
  values.push_back(double(stat.device_write_bytes) / hostB);       // Total WAF
  values.push_back(double(stat.device_user_write_bytes) / hostB);  // User WAF
  values.push_back(double(stat.device_gc_write_bytes) / hostB);    // GC WAF
}
