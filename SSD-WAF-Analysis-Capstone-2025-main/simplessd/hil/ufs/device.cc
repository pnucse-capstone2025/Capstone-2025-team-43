...
LUN::_LUN(bool b, uint8_t n, ConfigReader &cfg) : bWellknown(b), id(n) {
  ...
  snprintf((char *)inquiry + 8, 24, "SimpleSSD UFS Device");
}
...
Device::Device(DMAInterface *d, ConfigReader &c)
    : pDMA(d),
      ...
      lun(false, 0x00, c) {
  // Initialize Strings
  snprintf((char *)strManufacturer, sizeof(strManufacturer), "CAMELab");
  snprintf((char *)strProductName, sizeof(strProductName), "SimpleSSD UFS Device");
  snprintf((char *)strSerialNumber, sizeof(strSerialNumber), "000000000000");
  snprintf((char *)strOEMID, sizeof(strOEMID), "None");
  snprintf((char *)strProductRevision, sizeof(strProductRevision), "v02.01");
